...
import type { LawaStateMultiYearRecord, LawaTrendMultiYearRecord } from '../../types';
...
const LawaBrowsePage: React.FC = () => {
  ...
  const getTrendBadgeClass = (trendClassification: string) => {
                  ...
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        viewType === 'trend' && 'trendClassification' in row ? getTrendBadgeClass(row.trendClassification) : 
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {viewType === 'state' && 'stateNorm' in row ? row.stateNorm : 
                         viewType === 'trend' && 'trendClassification' in row ? row.trendClassification : 'UNKNOWN'}
                      </span>
                    </td>
                  </tr>
                ))
