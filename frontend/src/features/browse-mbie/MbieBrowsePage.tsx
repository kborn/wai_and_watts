...
import { useMbieGenerationAnnual, useMbieGenerationQuarterly } from '../../api/hooks';
import type { MbieGenerationAnnualRecord, MbieGenerationQuarterlyRecord } from '../../types';
...
const MbieBrowsePage: React.FC = () => {
  ...
  const annualData = useMbieGenerationAnnual({ fuelType: fuelType || undefined });
  const quarterlyData = useMbieGenerationQuarterly({ fuelType: fuelType || undefined });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="bg-white shadow rounded-lg p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">MBIE Electricity Generation Data</h1>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div>
            ...
            <select
              value={viewType}
              onChange={(e) => setViewType(e.target.value as 'annual' | 'quarterly')}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                data.map((row: MbieGenerationAnnualRecord | MbieGenerationQuarterlyRecord) => (
                  <tr key={row.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {viewType === 'annual' ? row.periodYear : `Q${(row as MbieGenerationQuarterlyRecord).periodQuarter} ${row.periodYear}`}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {row.fuelTypeRaw}
                    </td>
